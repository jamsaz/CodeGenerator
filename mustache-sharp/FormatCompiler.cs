...
namespace Mustache
{
    ...
    public sealed class FormatCompiler
    {
        ...
        public string OpenTagDelimeter { get; set; }
        public string CloseTagDelimeter { get; set; }
        ...
        private Regex prepareRegex(TagDefinition definition)
        {
            ...
            if (!_regexLookup.TryGetValue(definition.Name, out regex))
            {
                ...
                string match = (!string.IsNullOrEmpty(this.OpenTagDelimeter) ? (this.OpenTagDelimeter + "(") : "{{(") + String.Join("|", matches) + (!string.IsNullOrEmpty(this.CloseTagDelimeter) ? (")" + this.CloseTagDelimeter) : ")}}");
                //string match = ((this.OpenTagDelimeter != string.Empty) ? this.OpenTagDelimeter : "{{") + "(" + String.Join("|", matches) + ")" + ((this.CloseTagDelimeter != string.Empty) ? this.CloseTagDelimeter : "}}");
